...
async function desativarEntregador(ent) {
    ...
    const res = await fetch(`${API_BASE}/api/entregadores/${ent.id}/disable`, {
        method: 'PATCH',
    })
...
async function excluirEntregador(ent) {
    if (!confirm(`Excluir "${ent.nome}" PERMANENTEMENTE? Isso remove também os locais.`)) return
...
<template>
    <div class="bg-gray-800 p-5 rounded-lg border border-emerald-600">
        ...
        <div class="flex gap-2 mb-4">
            ...
            <button class="px-3 py-2 rounded bg-emerald-700 hover:bg-emerald-600 text-white text-sm"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div
                ...
                <button v-for="e in entregadores" :key="e.id"
                    ...
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <div class="flex gap-2">
                            <button class="px-2 py-1 rounded bg-gray-700 hover:bg-gray-600 text-white text-xs"
                                @click.stop="toggleAtivo(e)">
                                {{ e.ativo ? 'Desativar' : 'Ativar' }}
                            </button>

                            <button class="px-2 py-1 rounded bg-red-700 hover:bg-red-600 text-white text-xs"
                                @click.stop="excluirEntregador(e)">
                                Excluir
                            </button>
                        </div>
                    </div>
                        ...
                        <tbody>
                            <tr v-for="l in locaisFiltrados" :key="l.id" class="border-t border-gray-800">
                                ...
                                <td class="px-2 py-2 text-right">
                                    <button class="px-2 py-1 rounded bg-red-700 hover:bg-red-600 text-white"
