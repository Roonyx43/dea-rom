...
const mapCad = (item) => {
  ...
  return {
    ...
    // 🔹 Local e entregador vindos do backend
    ...
    status:
      (statusOrc === 'OC' || statusOrc === 'OA' || statusOrc === 'CD')
        ? 'Cadastrado'
        : statusOrc,
...
const bloqueadosSet = computed(() =>
  new Set(ticketsFinanceiroAll.value.map(t => String(t.codigo)))
)
